  ...
  return (
    <div className='operational-view'>
      ...
      <section className='app-panel overflow-hidden'>
        <div className='border-b border-[var(--border)] p-4 sm:p-5'><div className='relative max-w-xl'><label htmlFor='outbound-search' className='sr-only'>{t('outbound.searchLabel')}</label><Search size={17} className='pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)]' aria-hidden='true' /><input id='outbound-search' type='search' value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t('outbound.searchPlaceholder')} className='min-h-11 w-full rounded-[6px] border border-[var(--border-strong)] bg-[var(--surface-raised)] pl-10 pr-3 text-sm text-[var(--text)] outline-none placeholder:text-[var(--text-subtle)] focus:border-[var(--brand-orange)] focus:ring-4 focus:ring-[var(--brand-orange)]/10' /></div></div>
        <div className='overflow-x-auto'><table className='data-table min-w-[940px]'><caption className='sr-only'>{t('outbound.tableCaption')}</caption><thead><tr><th scope='col'>{t('outbound.dateRequester')}</th><th scope='col'>{t('common.partNumber')}</th><th scope='col'>{t('outbound.warehouseType')}</th><th scope='col' className='text-right'>{t('outbound.requestQty')}</th><th scope='col' className='text-right'>{t('outbound.supplyQty')}</th><th scope='col' className='text-right'>{t('outbound.outstanding')}</th><th scope='col'>{t('outbound.documents')}</th></tr></thead><tbody>{filtered.map((item) => { const outstanding = Math.max(0, item.qtyRequest - item.qtySupply); const documents = Object.entries(item.documents).filter(([, value]) => value).map(([key, value]) => `${key.toUpperCase()}: ${value}`).join(' | '); return <tr key={item.id}><td><p className='font-semibold text-[var(--text)]'>{formatDate(item.requestDate)}</p><p className='mt-1 text-xs text-[var(--text-muted)]'>{item.requester}</p></td><td><p className='font-semibold text-[var(--text)]'>{item.partNumber}</p><p className='mt-1 max-w-[260px] truncate text-xs text-[var(--text-muted)]'>{parts.find((part) => part.partNumber === item.partNumber)?.description ?? t('common.notAvailable')}</p></td><td><span className='text-xs text-[var(--text-muted)]'>{item.warehouseType}</span></td><td className='text-right font-semibold text-[var(--text)]'>{formatNumber(item.qtyRequest)}</td><td className='text-right font-semibold text-[var(--text)]'>{formatNumber(item.qtySupply)}</td><td className='text-right'>{outstanding > 0 ? <StatusBadge status='warning'>{formatNumber(outstanding)}</StatusBadge> : <StatusBadge status='ready'>0</StatusBadge>}</td><td><p className='max-w-[260px] text-xs leading-5 text-[var(--text-muted)]'>{documents || '-'}</p></td></tr>})}{filtered.length === 0 && <tr><td colSpan={7} className='py-16 text-center text-sm text-[var(--text-muted)]'>{t('outbound.noData')}</td></tr>}</tbody></table></div>
      </section>
      <Modal open={open} onClose={closeModal} title={t('outbound.modalTitle')} description={t('outbound.modalDescription')} size='lg'>
        <div className='space-y-6'>
          <div className='grid gap-4 sm:grid-cols-2'><TextField id='outbound-date' label={t('outbound.requestDate')} type='date' value={draft.requestDate} onChange={(value) => setDraft((current) => ({ ...current, requestDate: value }))} required /><TextField id='outbound-requester' label={t('outbound.requester')} value={draft.requester} onChange={(value) => setDraft((current) => ({ ...current, requester: value }))} placeholder={t('outbound.requesterPlaceholder')} required /><SelectField id='outbound-part' label={t('common.partNumber')} value={draft.partNumber} onChange={(value) => setDraft((current) => ({ ...current, partNumber: value }))} options={parts.map((part) => ({ value: part.partNumber, label: `${part.partNumber} | ${part.description.slice(0, 34)}` }))} required /><SelectField id='outbound-warehouse' label={t('outbound.warehouseType')} value={draft.warehouseType} onChange={(value) => setDraft((current) => ({ ...current, warehouseType: value as WarehouseType }))} options={warehouseOptions} required /><NumberField id='outbound-request-qty' label={t('outbound.requestQty')} value={draft.qtyRequest} onChange={(value) => setDraft((current) => ({ ...current, qtyRequest: value, qtySupply: Math.min(current.qtySupply, value) }))} min={1} required /><NumberField id='outbound-supply-qty' label={t('outbound.supplyQty')} value={draft.qtySupply} onChange={(value) => setDraft((current) => ({ ...current, qtySupply: value }))} min={0} required /></div>
