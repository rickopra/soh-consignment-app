...
const HEADERS = {
  ...
  USERS: ['id', 'username', 'email', 'displayName', 'role', 'passwordHash', 'passwordSalt', 'passwordIterations', 'mustChangePassword', 'active', 'failedAttempts', 'lockedUntil', 'lastLoginAt', 'passwordChangedAt', 'createdAt', 'updatedAt'],
  AUTH_SESSIONS: ['id', 'userId', 'tokenHash', 'purpose', 'createdAt', 'expiresAt', 'lastSeenAt', 'revokedAt', 'client'],
  AUTH_AUDIT: ['id', 'eventType', 'userId', 'username', 'outcome', 'details', 'createdAt'],
}
...
function doGet(event) {
  try {
    if (event.parameter && event.parameter.bridge === '1') return renderBridge_(event.parameter.nonce || '')
    ...
    return response_(handleRequest_({ action: 'BOOTSTRAP', token: event.parameter.token || '' }))
  } catch (error) {
    return response_({ error: error.message, code: error.code || 'REQUEST_FAILED' })
  }
}
...
function doPost(event) {
  try {
    ...
    if (String(body.action || '').toUpperCase() === 'PROVISION_INITIAL_ADMIN') return response_({ data: provisionInitialAdmin() })
    if (String(body.action || '').toUpperCase() === 'REISSUE_INITIAL_ADMIN') return response_({ data: reissueInitialAdminCredentials() })
    return response_(handleRequest_(body))
  } catch (error) {
    return response_({ error: error.message, code: error.code || 'REQUEST_FAILED' })
  }
}
